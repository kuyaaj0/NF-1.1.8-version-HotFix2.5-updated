...
class NoteOffsetState extends MusicBeatState
{
	...
	override public function update(elapsed:Float)
	{
		...
		if (onComboMenu)
		{
			...
			if (holdingObjectType != -1)
			{
				if (FlxG.mouse.justMoved || analogMoved)
				{
					...
					if (addNum != 4)
						...
						ClientPrefs.data.comboOffset[addNum + 1] = Math.round(startComboOffset.y + (mousePos.y - startMousePos.y));
					repositionCombo();
				}
			}
			...
			if (controls.RESET || virtualPad.buttonC.justPressed)
			{
				...
				for (i in 0...ClientPrefs.data.comboOffset.length)
				{
					...
					if (i == 4)
						ClientPrefs.data.comboOffset[i] = 530;
					if (i == 5)
						ClientPrefs.data.comboOffset[i] = 470;
				}
				repositionCombo();
			}
		}
		else
		{
			...
			{
				if (getData == 0)
					getData = Math.round(autoOffset * 1000);
				else
					getData = Math.round(getData * 0.9 + autoOffset * 1000 * 0.1);
				tipsTxt.text = 'Press SPACE to get suggest offset\nmore times press more data accurate\nsuggest offset: ${getData} MS';
			}
		}
	...
	function reloadTexts()
	{
		for (i in 0...dumbTexts.length)
		{
			switch (i)
			{
				...
				case 4:
					dumbTexts.members[i].text = 'KeyboardDisplay Offset:';
				case 5:
